        ...
        <div class="d-lg-none d-block">
            <form class="mobile-menu-form mb-5" action="{{ route('main.search') }}" method="post">
                ...
                <div class="input-with-btn d-flex flex-column">
                    ...
                    <button type="submit" class="eg-btn btn--primary btn--sm">{{ translate('Search') }}</button>
                </div>
            </form>
    ...
    <div class="nav-right d-flex align-items-center">
        ...
        <div class="eg-btn btn--primary header-btn">
            @if (Auth::check() && Auth::user()->role == 1)
                <a href="{{ route('customer.dashboard') }}">{{ translate('My Account') }}</a>
            @elseif(
                (Auth::check() && Auth::user()->role == 2) ||
                    (Auth::check() && Auth::user()->role == 3) ||
                    (Auth::check() && Auth::user()->role == 4))
                <a href="{{ route('backend.dashboard') }}">{{ translate('My Account') }}</a>
            @else
                <a href="{{ route('login') }}">{{ translate(get_setting('login_btn')) }}</a>
            @endif
        </div>
